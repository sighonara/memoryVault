...
def handler(event, context):
    """EventBridge-triggered scheduler. POSTs to the EC2 internal youtube-sync endpoint.
    ...
    try:
        with urllib.request.urlopen(req, timeout=30) as resp:
            ...
            print(f"YouTube sync triggered: status={resp.status} body={body}")
            return {"statusCode": resp.status, "body": body}
    except urllib.error.HTTPError as e:
        detail = e.read().decode("utf-8", errors="replace")
        print(f"YouTube sync HTTP error: status={e.code} body={detail}")
        raise
    except Exception as e:
        print(f"YouTube sync failed: {type(e).__name__}: {e}")
        raise
