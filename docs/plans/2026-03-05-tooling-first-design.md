...
- **WebSocket SockJS mismatch (Phase 8)** — Server's `WebSocketConfig.kt` registers the `/ws` endpoint with `.withSockJS()` but Angular's `WebSocketService` connects as raw WebSocket via `rx-stomp`. STOMP sessions never establish in production (Caddy logs show "connection reset by peer", server shows zero CONNECT sessions). Fix requires either removing `.withSockJS()` from the server or switching the Angular client to a SockJS transport.
- **Admin page 401 on cost queries (Phase 9F)** — Intermittent 401 on GraphQL cost queries in production causes auth interceptor cascade (clears token → redirects to login). Root cause unknown. Diagnostic logging added 2026-04-17: bumped `CognitoTokenValidator` failure log from DEBUG→WARN, added WARN in `CognitoJwtFilter` when validation returns null. Redeploy and reproduce to get visibility.
